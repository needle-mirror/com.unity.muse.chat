...
namespace Unity.Muse.Chat
{
    partial class WebAPI
    {
            ...
            Action<Exception> onError)
        {
            ...
            m_CurrentInspirationRequest = api.GetMuseInspirationV1Builder()
                .SetLimit(MuseChatConstants.MaxInspirationCount)
                .BuildAndSendAsync(this.m_CurrentInspirationCancellationTokenSource.Token);
            ...
            void RequestTick()
            {
                ...
                if (m_CurrentInspirationRequest.Result.Data != null)
                {
            ...
            Action<Exception> onError)
        {
            ...
            Configuration configuration = CreateConfig();
            MuseChatBackendApi api = new(configuration);
            var tsc = api.PostMuseInspirationV1Builder(inspiration)
        ...
        /// scope and return a conversation in response.
        /// </summary>
        internal void DeleteInspiration(Inspiration inspiration,
            ILoopRegistration loop,
            Action<Inspiration> onComplete,
            Action<Exception> onError)
        {
            ...
            var tsc = api.DeleteMuseInspirationUsingInspirationIdV1Builder(inspiration.Id)
                .BuildAndSendAsync(m_CurrentInspirationCancellationTokenSource.Token);
            ...
            void RequestTick()
            {
                ...
                if (tsc.IsCompletedSuccessfully)
                {
                    ApiResponse<ResponseDeleteMuseInspirationUsingInspirationIdV1> res = tsc.Result;
                    if (res == null)
                    {
                        onError?.Invoke(new WebAPIException("Inspiration Update Exception", res));
                        return;
